...
namespace DevionGames.UIWidgets
{
    public class UIWidgetEditor : EditorWindow
    {
        ...
        {
            UIWidgetEditor window = GetWindow<UIWidgetEditor>("UI Manager");
        ...
        private void OnGUI()
        {
            ...
            DisplayType selectedDisplayType =
                (DisplayType)EditorGUILayout.EnumPopup(displayType, EditorStyles.toolbarPopup);
            ...
            for(int i = 0; i < targets.Length; i++){
                ...
                if(GUILayout.Button(canvasGroup.alpha > 0.01f ? EditorGUIUtility.FindTexture("d_scenevis_hidden") : EditorGUIUtility.FindTexture("d_scenevis_visible"),
                                    EditorStyles.label)){
        ...
        private void FindWidgets()
        {
            ...
            for(int i = 0; i < targets.Length; i++){
                widgetPaths.Add(displayType == DisplayType.Path
                                    ? GetHierarchyPath(targets[i].transform)
                                    : targets[i].Name);
            }
