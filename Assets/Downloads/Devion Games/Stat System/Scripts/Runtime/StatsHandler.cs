...
namespace DevionGames.StatSystem
{
    public class StatsHandler : MonoBehaviour, IJsonSerializable
    {
        ...
        [StatPicker]
        [SerializeField]
        public List<Stat> m_Stats = new();
        [HideInInspector]
        [SerializeField]
        private List<StatOverride> m_StatOverrides = new();

        [SerializeField]
        protected List<StatEffect> m_Effects = new();
        ...
        private void SendDamage(GameObject receiver, Object data)
        {
            ...
            if (receiverHandler != null && receiverHandler.enabled && damageData != null)
            {
                ...
                if (damageData.hitSounds.Length > 0)
                    receiverHandler.PlaySound(damageData.hitSounds[Random.Range(0, damageData.hitSounds.Length)], damageData.audioMixerGroup, damageData.volumeScale);
        ...
        private void DisplayDamage(GameObject prefab, float damage, Color color, Vector3 intensity)
        {
            ...
            if (canvas != null)
            {
                ...
                go.transform.localPosition += new Vector3(Random.Range(-intensity.x, intensity.x), Random.Range(-intensity.y, intensity.y), Random.Range(-intensity.z, intensity.z));
