...
namespace DevionGames.StatSystem
{
    ...
    {
        [InspectorLabel("Name")]
        [SerializeField] private string m_HandlerName = string.Empty;
        public string HandlerName => m_HandlerName;
        public bool saveable;
        [StatPicker]
        [SerializeField] public List<Stat> m_Stats = new();
        [HideInInspector]
        [SerializeField] private List<StatOverride> m_StatOverrides = new();

        [SerializeField] protected List<StatEffect> m_Effects = new();
        ...
        private void Start()
        {
            for(int i = 0; i < m_Stats.Count; i++)
                m_Stats[i] = Instantiate(m_Stats[i]);

            if(m_StatOverrides.Count < m_Stats.Count){
                for(int i = m_StatOverrides.Count; i < m_Stats.Count; i++){
                    m_StatOverrides.Insert(i, new StatOverride());
                }
            }

            for(int i = 0; i < m_Stats.Count; i++)
                m_Stats[i].Initialize(this, m_StatOverrides[i]);

            for(int i = 0; i < m_Stats.Count; i++)
                m_Stats[i].ApplyStartValues();
        ...
        private void SendDamage(GameObject receiver, Object data)
        {
            ...
            if(receiverHandler != null && receiverHandler.enabled && damageData != null){
                Stat sendingStat = m_Stats.FirstOrDefault(x => x.Name == damageData.sendingStat);
                if(sendingStat == null)
                    return;

                Stat criticalStrikeStat = m_Stats.FirstOrDefault(x => x.Name == damageData.criticalStrikeStat);

                bool criticaleStrike = criticalStrikeStat != null && criticalStrikeStat.Value > Random.Range(0f, 100f);
                ...
                if(damageData.particleEffect != null){
                    Vector3 pos = receiver.GetComponent<Collider>()
                                          .ClosestPoint(transform.position + damageData.offset);
                    Vector3 right = Random.Range(-damageData.randomize.x, damageData.randomize.x) * transform.right;
                    Vector3 up = Random.Range(-damageData.randomize.y, damageData.randomize.y) * transform.up;
                    Vector3 forward = Random.Range(-damageData.randomize.z, damageData.randomize.z) * transform.forward;

                    Vector3 relativePos = (transform.position + damageData.offset + right + up + forward) - pos;
                    GameObject effect = Instantiate(damageData.particleEffect, pos,
                ...
                if(damageData.hitSounds.Length > 0)
                    receiverHandler.PlaySound(damageData.hitSounds[Random.Range(0, damageData.hitSounds.Length)],
                ...
                if(damageData.displayDamage)
                    receiverHandler.DisplayDamage(damageData.damagePrefab, damage,
                                                  criticaleStrike
                                                      ? damageData.criticalDamageColor
                                                      : damageData.damageColor, damageData.intensity);
                ...
                if(animator != null && animator.GetCurrentAnimatorStateInfo(0).IsName("Locomotion") &&
        ...
        private void DisplayDamage(GameObject prefab, float damage, Color color, Vector3 intensity)
        {
            ...
            if(canvas != null){
                ...
                go.transform.localPosition += new Vector3(Random.Range(-intensity.x, intensity.x),
        ...
        private void PlaySound(AudioClip clip, AudioMixerGroup audioMixerGroup, float volumeSclae)
        {
            ...
            m_AudioSource.PlayOneShot(clip, volumeSclae);
        }
        ...
        {
            return m_Stats.Find(x => x.Name == name);
        }
        ...
        {
            StatEffect instance = m_Effects.Find(x => x.Name == effect.Name);
            m_Effects.Remove(instance);
        }
