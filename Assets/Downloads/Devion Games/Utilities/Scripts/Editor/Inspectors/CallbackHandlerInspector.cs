...
namespace DevionGames
{
    ...
    public class CallbackHandlerInspector : Editor
    {
        ...
        protected virtual void OnEnable()
        {
            ...
            Type[] subInspectors = Utility.BaseTypesAndSelf(GetType())
                                          .Where(x => x.IsSubclassOf(typeof(CallbackHandlerInspector)))
                                          .ToArray();
            Array.Reverse(subInspectors);

            for(int i = 0; i < subInspectors.Length; i++){
                MethodInfo method = subInspectors[i]
                    .GetMethod("DrawInspector", BindingFlags.NonPublic | BindingFlags.Instance);
                Type inspectedType = typeof(CustomEditor)
                                     .GetField("m_InspectedType", BindingFlags.NonPublic | BindingFlags.Instance)
                                     .GetValue(subInspectors[i].GetCustomAttribute<CustomEditor>()) as Type;
                FieldInfo[] fields = inspectedType.GetAllSerializedFields()
                                                  .Where(x => !x.HasAttribute(typeof(HideInInspector)))
                                                  .ToArray();

                string[] classProperties =
                    fields.Where(x => x.DeclaringType == inspectedType).Select(x => x.Name).ToArray();
                    ...
                    m_DrawInspectors.Add(delegate()
                                             {
                                                 foreach(string classProperty in classProperties){
                                                     SerializedProperty property =
                                                         serializedObject.FindProperty(classProperty);
            ...
            string[] names = ((CallbackHandler)target).Callbacks;
        ...
        public override void OnInspectorGUI()
        {
            ...
            foreach(System.Action drawInspector in m_DrawInspectors){
        ...
        private void ShowAddEventmenu()
        {
            ...
            for(int i = 0; i < eventCallbackTypes.Length; i++){
                ...
                for(int j = 0; j < m_DelegatesProperty.arraySize; j++){
                    if(m_DelegatesProperty.GetArrayElementAtIndex(j).FindPropertyRelative("eventID").stringValue ==
                       eventCallbackTypes[i].text){
                        flag = false;
                    }
                }
